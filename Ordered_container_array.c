...
int OC_binary_search_helper(const struct Ordered_container* c_ptr, const void* search_ptr, int *insertion_position,
...
void OC_insert(struct Ordered_container* c_ptr, void* data_ptr)
{
    ...
    int item_position = OC_binary_search_helper(c_ptr, data_ptr, &insertion_position, c_ptr->comp_fun);
    if (item_position != -1)
    ...
    if (c_ptr->size > c_ptr->allocation) {
        ...
        /* copy the item_ptrs before insertion position to new array */
        for (i = 0; i < insertion_position; i++)
            new_array[i] = c_ptr->array[i];
        new_array[insertion_position] = data_ptr;
        
        /* copy the item_ptrs after insertion position to new array */
        ...
        /* push all the item_ptrs after the insertion position by 1 */
...
{
    int insertion_position;
    int item_position =OC_binary_search_helper(c_ptr, arg_ptr, &insertion_position, fafp);
    if (item_position == -1)
        return  NULL;
    else
        return c_ptr->array + item_position;
